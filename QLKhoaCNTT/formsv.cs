...
namespace QLKhoaCNTT {
    public partial class formsv : Form
    {
        ...
        private void dgvSinhVien_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                txtMaSV.Text = dgvSinhVien.Rows[dong].Cells[0].Value.ToString().Trim();
                txtTenSV.Text = dgvSinhVien.Rows[dong].Cells[1].Value.ToString().Trim();
                txtMaLop.Text = dgvSinhVien.Rows[dong].Cells[2].Value.ToString().Trim();
                cbKhoa.Text = dgvSinhVien.Rows[dong].Cells[3].Value.ToString().Trim();
            }
            catch
            {
                MessageBox.Show("Lỗi");
